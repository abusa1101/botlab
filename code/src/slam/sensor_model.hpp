...
/**
* SensorModel implements a sensor model for computing the likelihood that a laser scan was measured from a
* provided pose, given a map of the environment.
*
...
private:
    ...
    double isCellHit(double log_odds);
    double scoreRay(const adjusted_ray_t& ray, const OccupancyGrid& map);
    // float xCoord(float ray_range, float delta_range, float ray_theta, float rayStart_x);
    // float yCoord(float ray_range, float delta_range, float ray_theta, float rayStart_y);
};
