                                   ...
                                   const MotionPlanner& planner)
{
    ...
    */
    ...
    //
    // //If no fronters left, return error
    // if (frontiers.empty())
    // {
    //     path.path_length = 1;
    //     path.path.insert(path.path.begin(), robotPose);
    //     return path;
    // }
    //
    // //Find closest frontier to robot's current pose
    // double lowest_dist = 10000.0;
    // double dx, dy, dist;
    // int lowest_idx;
    //
    // //closest frontier is dist 'lowest_dist' away and 'lowest_idx'th element in frontiers vector
    // for (int i = 0; i < frontiers.size(); i++)
    // {
    //     dx = std::abs(frontiers[i].x - robotPose.x);
    //     dy = std::abs(frontiers[i].y - robotPose.y);
    ...
    // goal_pose.x = frontiers[lowest_idx].x;
    // goal_pose.y = frontiers[lowest_idx].y;
    // goal_pose.theta = 0.0;
    //
    // //Plan path to frontier
    // path = planPath(robotPose, goal_pose);
    //
